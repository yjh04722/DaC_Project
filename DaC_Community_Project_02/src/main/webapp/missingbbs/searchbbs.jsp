...
<body>
	...
	<tr>
     	...
     	<form method="post" name="search" action="searchbbs.jsp">
			<div class="search_aban">
			  <input type="text" name="searchText" placeholder="장소를 입력해 주세요." value="<%=searchText %>">
    ...
    <tr>
		...
		<%
			ArrayList<MissingBbs> list = missingBbsDAO.getSearch(searchText);
			if (list.size() == 0) {
	...
	%>
		    	    ...
		    	    <% if(startPage > pageBlock){ %>
		    	    	<li><a href="searchbbs.jsp?pageNum=<%=startPage-pageBlock %>&searchText=<%=searchText %>">Prev</a></li>
		    	    <% } %>
		    	    
		    	     <% for(int j = startPage; j <= endPage; j++){ %>
		    	    	<li><a href="searchbbs.jsp?pageNum=<%=j %>&searchText=<%=searchText %>"><%=j %></a></li>
		    	    <% } %>
		    	    
		    	     <% if(endPage < pageCount){ %>
		    	    	<li><a href="searchbbs.jsp?pageNum=<%=startPage+pageBlock %>&searchText=<%=searchText %>">Next</a></li>
		    	    <% } %>	    	    	
		    	    <%}  %>
		 </ul>
      	</td>
